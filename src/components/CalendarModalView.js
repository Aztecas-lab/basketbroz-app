...
const CalendarModalView = forwardRef((props, ref) => {
  ...
  const [selected, setSelected] = useState(props?.selectedDay ?? moment().format('YYYY-MM-DD'));
    ...
    >
      ...
      <Calendar
        ...
        onMonthChange={async (payload) => {
          ...
          const startAt = moment(dateString).startOf('month').format('YYYY-MM-DD');
          ...
          lastMonthChange.current = { 'month': payload.month, 'year': payload.year };
        }}
          ...
          if (direction === 'left') {
            return (
              <View style={{ opacity: disableLeftArrow ? 0 : 1 }}>
                <SVGIcon name={'ic_nav_back'} svgProps={{ height: 24, width: 24 }} />
              </View>
            );
          } else {
        ...
        dayComponent={({ date, state, onPress, marking }) => {
          ...
          return (
            ...
            >
              <Text style={{ fontSize: 16, textAlign: 'center', color: isDisable ? '#888' : '#fff' }}>{date.day}</Text>
              {hasGames && !isDisable ? (
                <View
                  style={{
                    position: 'absolute',
