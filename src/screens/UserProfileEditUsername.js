...
const UserProfileEditUsername = () => {
  const { authUser } = useAuthUser();
  const { updateUser } = useApi();
  const [input, setInput] = useState(authUser?.username);
  ...
  const onSavePress = useCallback(async () => {
    setIsLoading(true);
    const result = await updateUser({ username: input });
    setIsLoading(false);
    if (result.success) {
...
export default UserProfileEditUsername;
